...
# 加载图像并转换为灰度模式
blue_marble = Image.open("gebco.tif").convert("L")
...
output_image_north = cp.zeros((height, width, 2), dtype=cp.uint8)
output_image_south = cp.zeros((height, width, 2), dtype=cp.uint8)

# 将背景设置为完全透明
output_image_north[..., 1] = 0
output_image_south[..., 1] = 0
...
# 使用索引填充新图像
output_image_north[north_proj_y, north_proj_x, 0] = blue_marble_north
output_image_north[north_proj_y, north_proj_x, 1] = 255

output_image_south[south_proj_y, south_proj_x, 0] = blue_marble_south
output_image_south[south_proj_y, south_proj_x, 1] = 255

# 将CuPy数组转换为NumPy数组，然后转换为Pillow图像
output_image_north_pil = Image.fromarray(cp.asnumpy(output_image_north), mode='LA')
output_image_south_pil = Image.fromarray(cp.asnumpy(output_image_south), mode='LA')
...
def crop_to_circle(img):
    ...
    cropped_img = np.zeros((2*radius, 2*radius, 2), dtype=np.uint8)
    cropped_img[..., 1] = 0  # 初始化为完全透明
    ...
    return Image.fromarray(cropped_img, mode='LA')
